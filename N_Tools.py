...
def extract_table_grid(tex):
    ...
    h_candidates = []  # aspect-ratio says "horizontal rule-shaped": \hline, but
    v_candidates = []  # also \frac bars / minus signs (H) and matrix brackets (V)
    content = []

    # --- Step 1: classify glyphs by aspect ratio ---
    for i, g in enumerate(glyphs):
        w, h = g.width, g.height

        if h > 5 * w:  # vertical line
            v_candidates.append((i, g))
        elif w > 5 * h:  # horizontal line
            h_candidates.append((i, g))
        else:
            content.append((i, g))

    # Aspect ratio alone can't tell a real \hline/column-separator rule from a
    # \frac bar, a minus sign, or a matrix bracket -- all four are "thin and
    # elongated". The two axes need different discriminators though:
    #   - real \hline rules span the *entire table width*, while \frac bars and
    #     minus signs only span their cell's local content, so among H
    #     candidates the real rules are the widest ones.
    #   - real column-separator rules are drawn per table row, so their height
    #     is no help (a bmatrix bracket in that row is just as tall) -- but
    #     their *thickness* (width) is a fixed hairline (\arrayrulewidth)
    #     while a bracket glyph's stroke thickness scales with its height and
    #     is much thicker, so among V candidates the real rules are the
    #     thinnest ones.
    # Try to recover the exact expected counts from the source LaTeX so we
    # keep exactly that many; otherwise fall back to a size-relative cutoff.
    def expected_counts():
        latex = "".join(tex.tex_strings) if hasattr(tex, 'tex_strings') else tex.tex_string
        m = re.search(r'\\begin\{tabular\}\{([^}]*)\}(.*?)\\end\{tabular\}', latex, re.DOTALL)
        if not m:
            return None, None
        colspec, body = m.group(1), m.group(2)
        n_hlines = len(re.findall(r'\\hline', body))
        # Column-separator rules are drawn once per table row, not once for
        # the whole table, so the glyph count is per-row-count * per-row-bars.
        n_rows = max(n_hlines - 1, 1)
        return n_hlines, colspec.count('|') * n_rows

    def select_real_lines(candidates, extent_fn, expected_n, want_largest):
        if not candidates:
            return []
        ranked = sorted(candidates, key=lambda c: extent_fn(c[1]), reverse=want_largest)
        if expected_n is not None and expected_n <= len(candidates):
            return ranked[:expected_n]
        # Fallback: keep whatever is within 2x of the most extreme extent seen.
        best = extent_fn(ranked[0][1])
        return [c for c in candidates if extent_fn(c[1]) >= best / 2] if want_largest \
            else [c for c in candidates if extent_fn(c[1]) <= best * 2]

    n_hlines, n_vlines = expected_counts()
    horizontal_lines = select_real_lines(h_candidates, lambda g: g.width, n_hlines, want_largest=True)
    vertical_lines = select_real_lines(v_candidates, lambda g: g.width, n_vlines, want_largest=False)

    # Anything demoted back out of the line candidates (frac bars, minus
    # signs, matrix brackets, ...) is actual cell content.
    kept = {i for i, _ in horizontal_lines} | {i for i, _ in vertical_lines}
    content += [(i, g) for i, g in h_candidates + v_candidates if i not in kept]
    content.sort(key=lambda c: c[0])
